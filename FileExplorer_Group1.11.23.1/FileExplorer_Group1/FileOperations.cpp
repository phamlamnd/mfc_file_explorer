...
void CFileOperation::Size(CString sPathName)
{
	CFileFind finder;
	if (CheckPath(sPathName) == PATH_IS_FILE)
	{
		m_sizeFile = DoSize(sPathName);
		return;
	}
...
//Get size one file
ULONGLONG CFileOperation::DoSize(CString sPathName)
{
	CFile* pFile = NULL;
	...
	try
	{
		pFile = new CFile(sPathName, CFile::modeRead | CFile::shareDenyRead);
...
void CFileOperation::DoCopy(CString sSource, CString sDest, bool bDelteAfterCopy)
{
	...
	if (CheckPath(sSource) == PATH_IS_FILE && CheckPath(sDest) == PATH_IS_FOLDER) 
	{
		...
		CString unicodestr = sSource;
		CStringA charstr(unicodestr);
		_splitpath_s(charstr, drive, dir, name, ext);
	...
	else
	{
		...
		else
		{
			...
			else
			{
				...
				CString unicodestr = sCount;
				CStringA charstr(unicodestr);
				int iCount = atoi(charstr);
				iCount ++;
